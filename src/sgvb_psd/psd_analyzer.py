...
class psdanalyzer:
    def __init__(self, spec_true, spectral_density_q, n_freq, task_id, psd_estimator):
    ...
    def calculate_L2_error(self):
        ...
        for i in range(self.n_freq):
            N2_VI[i] = np.sum(np.diag((spec_mat_median[i,:,:] - self.spec_true[i,:,:]) @ 
    ...
    def calculate_CI_length(self):
        ...
        return len_point_CI_f11, len_point_CI_re_f12, len_point_CI_im_f12, len_point_CI_f22

    def calculate_coverage(self):
        spec_mat_lower_real = np.zeros_like(self.spectral_density_q[0], dtype=float)
        for j in range(self.n_freq):
            spec_mat_lower_real[j] = self._complex_to_real(self.spectral_density_q[0][j])

        spec_mat_upper_real = np.zeros_like(self.spectral_density_q[2], dtype=float)
        for j in range(self.n_freq):
            spec_mat_upper_real[j] = self._complex_to_real(self.spectral_density_q[2][j])

        coverage_point_CI = np.mean((spec_mat_lower_real <= self.real_spec_true) & (self.real_spec_true <= spec_mat_upper_real))
        return coverage_point_CI

    def run_analysis(self, iteration_start_time):
        L2_VI = self.calculate_L2_error()
        len_point_CI_f11, len_point_CI_re_f12, len_point_CI_im_f12, len_point_CI_f22 = self.calculate_CI_length()
