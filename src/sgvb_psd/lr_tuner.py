...
class OptimalSpectralDensityEstimator:
    ...
    1. Optimize the learning rate for determining MAP
    2. Optimize the surrogate parameters so that the ELBO is maximised (ie surrogate posterior == unnormlised posterior)
    2. use the optimal LR to estimate the optimal PSD (specifically

    IE. the phase where we optimize the posterior

    This is important because the LR can affect the
    convergence of the model during optimization of the
    unormalised posterior and the surrogate posterior
    """
    ...
    def compute_spectral_density(self, best_samp):
        ...
        theta_im_s = tf.matmul(
            Xmat_theta, tf.transpose(best_samp[4], [0, 2, 1])
        )
