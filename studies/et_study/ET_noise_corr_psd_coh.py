...
def objective(params):
    ...
    samp = result_list[2]
    if model_info == {}:
        model_info["Xmat_delta"], model_info["Xmat_theta"] = result_list[
            1
        ].Xmtrix(N_delta=N_delta, N_theta=N_theta)
...
spectral_density_q[:, :, diag_indices[0], diag_indices[1]] = np.quantile(
    np.real(spectral_density_all[:, :, diag_indices[0], diag_indices[1]]),
    [0.05, 0.5, 0.95],
    axis=0,
)
...
real_part = np.real(
    spectral_density_all[:, :, triu_indices[1], triu_indices[0]]
)
imag_part = np.imag(
    spectral_density_all[:, :, triu_indices[1], triu_indices[0]]
)

spectral_density_q[0, :, triu_indices[1], triu_indices[0]] = (
    np.quantile(real_part, 0.05, axis=0)
    + 1j * np.quantile(imag_part, 0.05, axis=0)
).T
spectral_density_q[1, :, triu_indices[1], triu_indices[0]] = (
    np.quantile(real_part, 0.50, axis=0)
    + 1j * np.quantile(imag_part, 0.50, axis=0)
).T
spectral_density_q[2, :, triu_indices[1], triu_indices[0]] = (
    np.quantile(real_part, 0.95, axis=0)
    + 1j * np.quantile(imag_part, 0.95, axis=0)
).T

spectral_density_q[:, :, triu_indices[0], triu_indices[1]] = np.conj(
    spectral_density_q[:, :, triu_indices[1], triu_indices[0]]
)
...
with h5py.File(
    "ETnoise_correlated_GP_pointwise_spec_matrices_XYZ.hdf5", "w"
) as f:
    f.create_dataset(
        "ETnoise_correlated_GP_spec_mat_median_XYZ", data=spec_mat_median
    )
    f.create_dataset(
        "ETnoise_correlated_GP_spec_mat_lower_XYZ", data=spec_mat_point_low
    )
    f.create_dataset(
        "ETnoise_correlated_GP_spec_mat_upper_XYZ", data=spec_mat_point_upp
    )
...
for i in range(n_samples):
    for j in range(n_freq):
        real_spectral_density_all[i, j] = complex_to_real(
            spectral_density_all[i, j]
        )
...
mad = median_abs_deviation(
    real_spectral_density_all, axis=0, nan_policy="omit"
)
mad[mad == 0] = 1e-10
...
with h5py.File(
    "ETnoise_correlated_GP_uniform_spec_matrices_XYZ.hdf5", "w"
) as f:
    f.create_dataset(
        "ETnoise_correlated_GP_spec_mat_median_XYZ", data=spec_mat_median
    )
    f.create_dataset(
        "ETnoise_correlated_GP_spec_mat_lower_XYZ", data=lower_bound
    )
    f.create_dataset(
        "ETnoise_correlated_GP_spec_mat_upper_XYZ", data=upper_bound
    )
...
for i in range(num_samples):
    for j in range(num_freq):
        ...
        squared_coherences[i, j] = (
            squared_matrix[np.triu_indices(p_dim, k=1)]
            / np.outer(diag, diag)[np.triu_indices(p_dim, k=1)]
        )

squared_coherences_stats = np.percentile(
    squared_coherences, [5, 50, 95], axis=0
)
...
with h5py.File(
    "ETnoise_correlated_GP_pointwise_squared_coh_XYZ.hdf5", "w"
) as f:
    f.create_dataset(
        "ETnoise_correlated_GP_coh_median_XYZ", data=squared_coherences_median
    )
    f.create_dataset(
        "ETnoise_correlated_GP_coh_lower_XYZ",
        data=squared_coherences_point_low,
    )
    f.create_dataset(
        "ETnoise_correlated_GP_coh_upper_XYZ",
        data=squared_coherences_point_upp,
    )
