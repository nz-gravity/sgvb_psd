...
class VarmaSim:
    ...
    def simData(self, vmaCoef, sigma=np.array([1.0])):
        n = self.n
        ...
        for i in np.arange(
            ...
            )  # itration for wt, wt-1, wt-2
            x[i,] = np.sum(
                np.matmul(vmaCoef, epsilon[..., np.newaxis]), axis=(0, -1)
            )
        x = x[101:]
        return x

    def calculateSpecMatrix(
        self, f, vmaCoef, sigma=np.array([1.0]), inverse=False
    ):
        specTrue = np.apply_along_axis(
            lambda f: self.calculateSpecMatrixHelper(
                f, vmaCoef, sigma, inverse=inverse
            ),
            axis=1,
            arr=f.reshape(-1, 1),
        )
    ...
    def calculateSpecMatrixHelper(
        ...
        A_f_re_ma = np.sum(
            vmaCoef * np.cos(np.pi * 2 * k_ma * f)[:, np.newaxis, np.newaxis],
            axis=0,
        )
        A_f_im_ma = -np.sum(
            vmaCoef * np.sin(np.pi * 2 * k_ma * f)[:, np.newaxis, np.newaxis],
            axis=0,
        )
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        ax[i].plot(
            f, np.log(Pxx_den0), marker=".", markersize=2, linestyle="-"
        )
        ax[i].plot(
            freq,
