...
def objective(params):
    ...
    Spec = SpecVI(x)
    result_list = Spec.runModel(N_delta=30, N_theta=30, lr_map=lr_map, ntrain_map=10000, sparse_op=False,
...
best = fmin(objective, space, algo=algo, max_evals=10)
...
spectral_density_q[:, :, diag_indices[0], diag_indices[1]] = np.quantile(spectral_density_all[:, :, diag_indices[0], diag_indices[1]], [0.05, 0.5, 0.95], axis=0)
